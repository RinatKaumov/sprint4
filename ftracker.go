...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speed := meanSpeed(action, duration)
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(speed*kmhInMsec, 2)/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH
	//((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах)* 0.029 * ВесСпортсменаВКг) * ВремяТренировкиВЧасах * minInH)
	//walkingCAloriesWeightMultiplier*weight + ((math.Pow(speed*kmhInMsec), 2))/(height/100))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	speed := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (speed + 1.1) * swimmingCaloriesWeightMultiplier * weight * duration //(СредняяСкоростьВКм/ч + 1.1) * 2 * ВесСпортсменаВКг * ВремяТренировкиВЧасах
}
